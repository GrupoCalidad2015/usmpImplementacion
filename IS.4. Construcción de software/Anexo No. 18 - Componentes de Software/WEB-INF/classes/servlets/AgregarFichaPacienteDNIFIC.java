...
/**
 * Servlet implementation class AgregarFichaPacienteDNIFIC
 */
@WebServlet("/AgregarFichaPacienteDNIFIC")
public class AgregarFichaPacienteDNIFIC extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public AgregarFichaPacienteDNIFIC() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			if(email.length()==0){
				...
				if (dni.length() == 0 && nombre.length() == 0
				...
				} else if (numEmergencia.length() < 15
					...
					request.getRequestDispatcher("/mensaje.jsp").forward(request,
						...
						String dniFic = fichaPaciente1.generarDNIFicticio();
					      
					
					FichaPacienteBean fichaPaciente = new FichaPacienteBean();
					fichaPaciente.setDni(dniFic);
					...
					if (flag) {
						request.setAttribute("msj", "Su registro fue satisfactorio,el DNI GENERADO ES:"+dniFic);
						request.getRequestDispatcher("/mensaje.jsp").forward(
								request, response);
					} else {
